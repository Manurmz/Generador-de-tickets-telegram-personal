...
def extraer_datos_mercado_pago_nuevo(textos: List[str]) -> Dict[str, Any]:
    """
    Extrae datos de recibos Mercado Pago en el NUEVO formato.
    Detección: "Comprobante de pago" + "Código de autenticación" + fecha en español.
    """
    if "Comprobante de pago" not in textos or "Código de autenticación" not in textos:
        return {}

    resultado = {}

    try:
        idx_comprobante = textos.index("Comprobante de pago")
    except ValueError:
        return {}

    try:
        idx_auth = textos.index("Código de autenticación")
        if idx_auth + 1 < len(textos):
            resultado["folio"] = textos[idx_auth + 1]
    except ValueError:
        pass

    try:
        idx_referencia = textos.index("Referencia")
        if idx_referencia + 1 < len(textos):
            resultado["referencia"] = textos[idx_referencia + 1]
    except ValueError:
        try:
            idx_telefono = textos.index("Número de teléfono")
            if idx_telefono + 1 < len(textos):
                resultado["referencia"] = textos[idx_telefono + 1]
        except ValueError:
            pass
    ...
    if idx_dollar != -1:
        dollar_token = textos[idx_dollar]
        numeros = re.findall(r'\d+', dollar_token)
        if numeros:
            resultado["monto"] = int(numeros[0])
        if idx_dollar + 1 < len(textos) and textos[idx_dollar + 1] == "00":
            pass

    dias_semana = {"lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo"}
    servicio_parts = []
    for i in range(idx_comprobante + 1, len(textos)):
        t = textos[i]
        if t.startswith("$") or t == "Comprobante":
            break
        primer_palabra = t.split(",")[0].lower().strip().rstrip(".")
        if primer_palabra in dias_semana:
            break
        if t.strip():
            servicio_parts.append(t)
    if servicio_parts:
        resultado["servicio"] = " ".join(servicio_parts).strip()

    fecha_token = None
    fecha_idx = -1
    for i, t in enumerate(textos):
        if re.match(r'.+,\s*\d+\s+de\s+\w+\s+de\s+\d+.*', t):
            fecha_token = t
            fecha_idx = i
            break

    if fecha_idx != -1:
        for i in range(fecha_idx + 1, len(textos)):
            t = textos[i]
            if t.startswith("$") or t == "00":
                continue
            if t.strip() and t != "Comprobante":
                resultado["servicio"] = t.strip()
                break

    if fecha_token:
        hora_match = re.search(r'(\d{1,2}):(\d{2})', fecha_token)
        if hora_match:
            hora_str = hora_match.group(0).rstrip('.')
            dia_match = re.search(r'(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})', fecha_token)
            if dia_match:
                dia = dia_match.group(1).zfill(2)
                mes_nombre = dia_match.group(2).lower()
                año = dia_match.group(3)
                mes = obtener_json.MESES.get(mes_nombre)
                if mes:
                    resultado["hora"] = f"{año}-{mes}-{dia} {hora_str}:00"
                    try:
                        resultado["hora"] = datetime.strptime(resultado["hora"], "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d %H:%M:%S")
                    except:
                        pass
...
def procesar_array(array: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Intentar extractor NUEVO formato siempre al final (sobreescribe si detecta su formato)
    resultado_nuevo = extraer_datos_mercado_pago_nuevo(textos)
    if resultado_nuevo:
